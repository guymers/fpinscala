...
object Nonblocking {
  ...
  object Par {
    ...
    def run[A](es: ExecutorService)(p: Par[A]): A = {
      ...
      val ref = new AtomicReference[Either[Throwable, A]] // A mutable, threadsafe reference, to use for storing the result
      ...
      try {
        p(es)(a => { ref.set(Right(a)); latch.countDown() }, onError)
      } catch {
    ...
    def async[A](f: (A => Unit) => Unit): Par[A] = es => new Future[A] {
      def apply(k: A => Unit, onError: Throwable => Unit) = f(k)
    }
